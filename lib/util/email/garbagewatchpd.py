...
class garbagewatchEPDaemon(daemon.Daemon):

	default_conf = '/etc/garbagewatch/garbagewatchpd.conf'
	section = 'garbagewatchpd'
	def run(self):
		logging.info("garbagewatch Email Parser Daemon is starting up")
...
def refresh():
    global cfg
	try:
	    epo = emailparser("imap.gmail.com",993,cfg.email_login,cfg.email_pass,"Inbox","smtp.gmail.com",587,cfg.email_pass)
        epo.process()		
	except:
...
class emailparser:
    ...
    def process(self):
		...
		try:
        	...
        	imapconn.logout()
            smtpconn.quit()
		    logging.info("garbagewatch Email Login Problem")
			return
		...
		try:
            ...
            imapconn.logout()
            smtpconn.quit()
            logging.info("garbagewatch Email Select Problem")
            return
		...
		try:
            ...
            imapconn.logout()
            smtpconn.quit()
            logging.info("garbagewatch Email Search Problem")
            return
        ...
        if status == 'OK':
            ...
            for message in messages[0].split():
    			...
    			try:
    		        ...
    		        imapconn.logout()
                    smtpconn.quit()
    		        logging.info("garbagewatch Email Fetch Problem")
                    return
    			...
    			try:
                    ...
                    imapconn.logout()
                    smtpconn.quit()
                    logging.info("garbagewatch Email Fetch 2 Problem")
                    return
                    ...
                    if((type == 'image/gif') or (type == 'image/jpeg') or (type == 'image/png')):
        				...
        				if(validimg and (not validate.spam(emailaddy))):
        					try:
        						...
        						msg.attach(MIMEText("- Camera phone upload powered by Mobile Garbage Watch"))
        						msg.attach(imgpart)
                        ...
                        if deleteme:
                            ...
                            for message in deleteme:
            					try:
                                	...
                                	imapconn.logout()
                                	smtpconn.quit()
                                	logging.info("garbagewatch Email Delete Problem")
                                	return
                    imapconn.close()
                    imapconn.logout()
                    smtpconn.quit()
