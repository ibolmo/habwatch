...
var SlidingTabs = new Class({
  ...
  initialize: function(buttonContainer, slideContainer, options) {
    ...
    if (this.options.animateHeight) this.heightFx.set(this.panes[this.current].offsetHeight);
  ...
  recalcWidths: function() {
    this.panes.setStyle('width', this.outerSlidesBox.offsetWidth.toInt() - this.options.rightOversized + 'px');
    ...
    // fix positioning
    if (this.current > 0) this.outerSlidesBox.scrollTo(this.current * this.outerSlidesBox.offsetWidth.toInt(), 0);
  }
});
