CREATE TABLE datum (id BIGSERIAL, file BYTEA, sf_guard_user_id INT, created_at TIMESTAMP without time zone, updated_at TIMESTAMP without time zone, PRIMARY KEY(id));
CREATE TABLE message (id BIGSERIAL, file BYTEA, sf_guard_user_id INT, _from VARCHAR(125), _to VARCHAR(125), message TEXT, type VARCHAR(255), carbon_copy TEXT, blind_carbon_copy TEXT, subject TEXT, created_at TIMESTAMP without time zone, updated_at TIMESTAMP without time zone, PRIMARY KEY(id));
CREATE TABLE course (id BIGSERIAL, file BYTEA, sf_guard_user_id INT, speed FLOAT, heading FLOAT, heading_accuracy FLOAT, speed_accuracy FLOAT, g_p_s_id BIGINT, created_at TIMESTAMP without time zone, updated_at TIMESTAMP without time zone, PRIMARY KEY(id));
...
CREATE TABLE picture (id BIGSERIAL, file BYTEA, sf_guard_user_id INT, resolution BIGINT, quality VARCHAR(10), format VARCHAR(25), created_at TIMESTAMP without time zone, updated_at TIMESTAMP without time zone, PRIMARY KEY(id));
CREATE TABLE position_table (id BIGSERIAL, file BYTEA, sf_guard_user_id INT, latitude FLOAT, altitude FLOAT, vertical_accuracy FLOAT, longitude FLOAT, horizontal_accuracy FLOAT, g_p_s_id BIGINT, created_at TIMESTAMP without time zone, updated_at TIMESTAMP without time zone, PRIMARY KEY(id));
...
CREATE TABLE satellites (id BIGSERIAL, file BYTEA, sf_guard_user_id INT, horizontal_dop FLOAT, used_satellites BIGINT, vertical_dop FLOAT, time FLOAT, satellites BIGINT, time_dop FLOAT, g_p_s_id BIGINT, created_at TIMESTAMP without time zone, updated_at TIMESTAMP without time zone, PRIMARY KEY(id));
